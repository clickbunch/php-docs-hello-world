...
if ($_GET['l']) {
	...
	foreach ($offers as $offer) {
		if ($offer->{'id'} == $offerid) {
			...
			if ($show_one == 0) { $show_one = ""; }
			$delimiter = $offer->{'delimiter'};
			$show_numbers = $offer->{'show_numbers'};
			$show_sub = $offer->{'show_sub'};
			$special = $offer->{'special_clickid'};
	...
	//Handler for stupid ass Casino Offers that can't get their shit together
	...
	if ($s3 != "") { $d_s3 = $delimiter . $subtag . $three . $s3; } else { $d_s3 = ""; }
	if ($s4 != "") { $d_s4 = $delimiter . $subtag . $four  . $s4; } else { $d_s4 = ""; }

	if ($s2 != "") {
		if ($special == "0" || $special == "") {
			$d_s2 = $delimiter . $subtag . $two   . $s2;
		} else {
			$d_s2 = $delimiter . $special . $two  . $s2;
		}
	} else {
		$d_s2 = "";
	}

	$link = $first . $s1 . $d_s2 . $d_s3 . $d_s4;
	$link = $olink . $link;
